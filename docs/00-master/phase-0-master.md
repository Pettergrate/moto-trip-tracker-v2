...
`REC-004` (user stop / force stop) covers the other way a capture gets orphaned: the user ends the app on purpose (Task Manager Stop, Force stop), after which Android does not restart the service and the capture would sit ACTIVE, recording nothing and blocking every new Start. At each process start an `ApplicationExitInfo` check (a cursor prevents acting on the same exit twice) seals the capture - same rules as a reboot: last persisted evidence, visible partial Trip - only when the previous process was ended with `REASON_USER_REQUESTED`; crashes and kills are left to the sticky service as before, and nothing is revived. Done, 394 unit tests, and on the real phone both a `stop-app` (Android logged `USER REQUESTED / STOP APP`) and a Settings-style `force-stop` (`USER REQUESTED / FORCE STOP`) sealed a trip, while a `kill -9` control (`SIGNALED`) still resumed. Those device runs found two real bugs: a Force stop left two identical partial Trips (two overlapping reconciliations; the seal now revalidates `ACTIVE` inside its transaction, with a concurrency test that fails without the guard), and `BOOT_COMPLETED` turned out to also arrive when a Force-stopped app is relaunched without any reboot, so `REC-003` now decides "real reboot" from `Settings.Global.BOOT_COUNT` (its earlier claim is corrected in the backlog; a second real reboot was verified). The user gets no explicit prompt beyond the visible partial trip; that and the OEM caveat are in the backlog.

`REC-005` (GPS gap and location-degraded handling) is the live half of something the pipeline already did after the fact: `ProcessingEngine` derived `LocationGap`s from the raw points, but while a trip was recording nothing noticed that fixes had stopped, so the notification and Active Trip screen read as healthy. A pure `LocationSignalWatch` now turns silence on the monotonic clock into `LOCATION_GAP_STARTED`/`LOCATION_GAP_ENDED` events (reason `NO_FIX`, or `LOCATION_SERVICES_OFF` when Location is off at that moment), a "No GPS signal" notification and an Active Trip card, using the same 30 s threshold the derived gaps use so live and post-trip disclose one definition. It never ends, pauses or fills in the trip (§13.2); a paused trip is never read as a lost signal; a gap still open at Finish is closed. Done, 425 unit tests, and on the real phone cutting only my own app's location appops produced a live gap after ~37 s (banner + notification), restoring them closed it (171 s), and Finish during a gap closed it cleanly; Trip Detail's derived "1 signal gap" agreed. The Location-Services-off reason was also verified on the phone on 2026-09-26, with the owner switching the toggle (card and notification appeared ~36 s after switching off and cleared when it came back; the events are in the database). An approximate-only permission downgrade was left unflagged at first and handled in a follow-up (`ADR-022`, below); both are in the backlog, as is a pre-existing mislabel where a long manual pause shows as a "signal gap".

`REC-006` (persistence-failure buffer and retry) replaces what `TRK-002` deliberately left undone: a raw point that failed to insert was logged as one error event and dropped, with no retry, one event per point, and nothing told the rider. A single `RawPointWriter` is now the only path a point takes to Room. On failure it holds points in a bounded, ordered buffer (overflow drops the oldest and says so), retries with a backoff so a dead database is not hammered, writes the held points back in order with their original sequence numbers, flushes on Finish before the Finish transaction reads the last sequence number, and records anything still unsaved as lost (`DATA_LOSS_DETECTED`) instead of leaving it looking saved. An outage is one episode with a handful of events, and the rider is told through the notification and an Active Trip card fed by an in-memory bus, because the database is the one place that warning cannot be stored; Trip Detail notes a trip whose recording lost points. Done, 486 unit tests. The failure path was verified with real Room and failures injected at the DAO and then, on 2026-09-26, on the phone itself through a debug-only fault injector that makes raw writes fail on demand (no implementation exists in release): a transient failure, a full disk, an overflow of a 10-point buffer and a Finish while the database was still failing all behaved as designed and were read back from the database (207 points with no holes across two outages; 43 holes matching exactly the 43 dropped on overflow; the loss recorded and shown in Trip Detail). Two defects the device showed were fixed. A corrupt or unopenable database (§14.4) is not handled, and only raw-point writes are covered; both are in the backlog.

**REC-005 follow-up (2026-09-26, `ADR-022`)** - what the phone showed when I revoked only the precise location permission from a running trip: the recording carried on and accepted a 2000 m fix ~920 m off, so a phone sitting still showed 1,0 and then 1,9 km, with the rider told only "No GPS signal". Each raw point now carries `isApproximateLocation` (schema v3, an additive nullable migration; everything recorded before stays unknown). The point is kept (ADR-006) but marked; processing rejects it and the live distance skips it; it is not signal; and the rider is told the real cause (notification and Active Trip card) while the trip keeps recording. The migration runs on the owner's real database, so it was proven on the JVM against the exported schema, dry-run on a copy of the real data (same counts and content checksums, integrity ok), then applied on the phone after a backup. Re-running the same scenario: distance stayed 0,0 km, and the finished trip has 19,8 m instead of a phantom kilometre, with the 2 approximate points rejected and the 80 precise ones accepted. 476 unit tests. `processingVersion` was deliberately not bumped (no existing point carries the marker). The instrumented migration test is written but not run, because `connectedAndroidTest` once wiped the production database.

`DIA-004` (process exit and recovery evidence) keeps how earlier processes ended - the platform remembers only a short ring, and differently per OEM - as `PROCESS_EXIT` diagnostic events recorded at every start (reason, importance, exit status; never the platform's free-text description). Before dying, the tracking service leaves a compact PII-free state hint (`cap=Y;pause=N;sig=OK;db=OK...`) that the platform hands back with the exit, so a row can say `SIGNALED while recording` instead of only `SIGNALED`. The diagnostic table now has the retention F0.13 asked for (14 days, 20,000 events, daily), with one deliberate exemption: `DATA_LOSS_DETECTED` rows, because a note in Trip Detail depends on them. Done, 511 unit tests; on the phone a `kill -9` and a force stop during a recording were both recorded with their state, REC-004 still sealed the capture, and the daily purge is scheduled. The purge itself has not run on the device (WorkManager will not run a periodic job early, and nothing is old enough yet), and crash/ANR reasons were not induced.

`DIA-002` (internal debug screen) is Settings -> "Diagnostics (internal)": the app's own state - health, build, capabilities, detector, location, tracking, processing, recovery - and the diagnostic timeline, filterable by category and severity and searchable by reason code, without `adb`. It is a read-only view over evidence that lives elsewhere, built from one `DiagnosticSnapshot` that `DIA-003` will reuse for its export. It never shows a coordinate (a fix is described by its age, accuracy and interval; a capture by a short SHA-256 label, not its id), and a test enforces that on the snapshot's shape. Where the app has no source it says so: the detector's "current state" is not persisted, so it shows what was recorded rather than inventing one. Done, 540 unit tests; on the phone the timeline counts matched the database exactly (157 events, 33 at WARN or above, 2 for a search), the capture label equalled the hash of the real id, none of 10 real coordinate values appeared in any line of the screen, and under an injected write failure the health state went DEGRADED, then PERSISTENCE_CRITICAL for a full disk, then HEALTHY. The Detector section was only seen empty (Auto Tracking is off) and the export action is `DIA-003`.

`DIA-003` (sanitized diagnostic ZIP) adds "Export diagnostic" to the debug screen: a package of the app's state and event history (`manifest`, versions, capabilities, health snapshot, worker state, process exits, `diagnostic-events.jsonl`, README) made from the same snapshot the screen shows. The standard package contains no coordinates, raw track, trip names or notes, and ids appear only as short one-way labels; route data is a separate opt-in (unticked by default, with a warning about precise locations) that the builder honours only when explicitly on. It is written to the app's private cache and handed to the system share sheet - the app uploads nothing. Done, 560 unit tests; on the phone I pulled a real package apart and checked it against the real database: 8,419 distinct coordinate values and 68 real ids of every ride recorded, none found in the standard package, and with route data on the coordinates appear only inside `route/` (12 points, matching the database). Not checked against real data: custom trip names and notes (this database has none), and I did not actually send the file to any app.
