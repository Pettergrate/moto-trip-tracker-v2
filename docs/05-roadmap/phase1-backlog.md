...
**Not yet verified — the phone was disconnected before this task started:** real FLP fixes have not been observed end-to-end on hardware (real GPS behavior, real permission enforcement against the actual `LocationRequest`, real notification/foreground-service interaction while location updates are live). `TRK-001`'s on-device pass exercised the same service/manifest/permission plumbing this task depends on, but not the location stream itself. This is a genuine gap, not a formality — flagging it rather than claiming verification that didn't happen. Do this the next time the device is connected, before treating TRK-002 as fully closed.
