...
Verified: `./gradlew assembleDebug testDebugUnitTest` green (240/240, including 7 new `FieldTestHarnessViewModelTest` cases covering blank-profile-id rejection, capability-mode resolution against real `CapabilityInputs`, a real active capture's info surfacing correctly, marker-count accumulation, and a full stop→export round trip whose `session.json`/`annotations.json` are parsed back with `org.json` to confirm the written content, not just that a file exists). On-device (Honor DNY-NX9): installed and confirmed the real navigation path Home → Settings → "Field test harness (internal)" → the Configuring screen, including the required-field gating (Start disabled until `experimentProfileId` is non-blank) and real text input.

**Honestly not verified live on-device:** the Active-state screen and the Stop & export tap. Twice, tapping "Start session" at its real on-screen coordinate brought WhatsApp to the foreground instead (`topResumedActivity` became `com.whatsapp/.home.ui.HomeActivity`) - this device has heavy WhatsApp notification traffic, and the same class of interference was already documented during UI-001. No Trip or capture data was affected either time (confirmed via Home's unchanged Recent list), and nothing was sent or touched inside WhatsApp. Given it recurred at the same spot, further blind coordinate-tap attempts were stopped rather than risking a real interaction with the user's WhatsApp; the Active/export path is exercised end-to-end by the unit tests above instead. Real on-device confirmation of that specific screen is deferred to whenever this harness gets used for an actual pilot ride (its real purpose), rather than forced here.

**Still not done, and the actual point of this task's title:** real pilot rides. The harness now exists; nobody has ridden with it yet.
