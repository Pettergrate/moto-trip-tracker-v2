...
class FieldTestSessionExporterTest {
    ...
    fun exportWritesSessionAndAnnotationsFiles() {
        ...
        exporter.export(metadata, markers, emptyList())
    ...
    @Test
    fun exportAlwaysWritesARawTrackCsvEvenWithNoPoints() {
        val writer = FakeFieldTestDatasetWriter()
        val exporter = FieldTestSessionExporter(writer)
        val metadata = FieldTestSessionMetadata(
            sessionId = "FT-2026-003",
            startedAt = 1_000L,
            endedAt = 9_000L,
            appVersion = "0.1-w0",
            diagnosticSchemaVersion = 1,
        ...
        exporter.export(metadata, emptyList(), emptyList())

        val csv = writer.readFile("FT-2026-003", "raw-track.csv")
        assertNotNull("raw-track.csv should exist even with zero points - an honest empty file, not a missing one", csv)
        assertEquals("sequenceNumber,capturedAt,elapsedRealtimeNanos,receivedAtElapsedRealtimeNanos,latitude,longitude,horizontalAccuracyM,altitudeEllipsoidM,altitudeMslM,verticalAccuracyM,speedMps,speedAccuracyMps,bearingDeg,bearingAccuracyDeg,provider,isMock,requestProfileId,callbackBatchId,detectorStateSnapshot", csv)
    }
}
