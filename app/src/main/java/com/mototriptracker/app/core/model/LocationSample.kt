 ...
 * contract — everything except the capture-bookkeeping fields
 * (captureId/sequenceNumber/requestProfileId/detectorStateSnapshot) that
 * only make sense once a capture is actively recording. This is what
 * TST-001's `LocationReplaySource` (app/src/test/...) feeds into detector/
 * processing logic during tests, per F0.12 §5.2's field list. Pure data,
 * zero Android dependency (ADR-013) — the DET and PRC tasks convert real
 * `android.location.Location` objects into this shape at the platform
 * boundary; this class itself never does.
 *
